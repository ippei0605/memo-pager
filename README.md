...
表示している先頭行のメモを削除した場合、ページインデックスに保存した startkey に対応するデータが無くなってしまいますが、Cloudant NoSQL DB は startkey が一致しなくても直後のデータから limit 分のデータを取得するので、ページインデックスを書き換えるなどの特別な処理はしてません。
